...
(function (sandbox) {
    ...
    function transformAst(object, visitorPost, visitorPre, context, noIgnore) {
        ...
        // 描述一个提前处理会遇到的问题：
        // window.location.href 如果提前处理MemberExpression的话，则是先处理'window.location'.'href',再处理'window'.'location',
        // 处理完完成后变成了J$.G(window.location, 'href')，然后下面的遍历object.key则会将J$.G进行处理，导致无限循环。
        // 这里的解决方案是，在遇到J$.G时不进行处理，这样就不会越来越多了。
        if (visitorPre && HOP(visitorPre, type)) {
            object = visitorPre[type](object, context);
        }
